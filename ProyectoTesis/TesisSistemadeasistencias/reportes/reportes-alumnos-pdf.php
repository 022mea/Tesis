...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(60);
        // Título
        $this->Cell(70, 10,  utf8_decode('Reporte de Alumnos'), 0, 0, 'C');
        // Salto de línea
        $this->Ln(15);

        /*-----------Encabezado------------*/
        $this->Cell(10, 10, utf8_decode('N°'), 1, 0, 'C', 0);
        $this->Cell(30, 10, utf8_decode('Nombres'), 1, 0, 'C', 0);
        $this->Cell(30, 10, utf8_decode('Apellidos'), 1, 0, 'C', 0);
        $this->Cell(30, 10, utf8_decode('Cedula'), 1, 0, 'C', 0);
        $this->Cell(15, 10, utf8_decode('Edad'), 1, 0, 'C', 0);
        $this->Cell(30, 10, utf8_decode('Direccion'),1, 0, 'C', 0);
        $this->Cell(20, 10, utf8_decode('Curso'), 1, 0, 'C', 0);
        $this->Cell(20, 10, utf8_decode('Paralelo'), 1, 1, 'C', 0);
    }
    ...
    function obtenerConsultas($errores, $campo)
    {
        ...
        if (isset($errores[$campo]) && !empty($campo)) {
            ...
            while ($row = $resultado->fetch_assoc()) {
                ...
                $pdf->Cell(30, 10, utf8_decode($row['nombres']), 1, 0, 'C', 0);
                $pdf->Cell(30, 10, utf8_decode($row['apellidos']), 1, 0, 'C', 0);
                ...
                $pdf->Cell(20, 10, utf8_decode($row['curso']), 1, 0, 'C', 0);
                $pdf->Cell(20, 10, utf8_decode($row['paralelo']), 1, 1, 'C', 0);
            }
            $pdf->Output();
        }
        return $alerta;
